...
plot_A <- 
  ...
  geom_segment(data=infctn[infctn$host=="BALB/c",], aes(x=4.5, y=0.5, xend=4.5, yend=1), linetype="dashed",
...
plot_B <- 
  ...
  ylab("Total worms") +
  facet_wrap(~host, strip.position = 'bottom') + 
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=4.5, y=0, xend=4.5, yend=31), linetype="dashed",
               color = "black", size=0.5) +
  my_theme() +
  add_pvalue(p_worms, tip.length = 0.01, label = {"p.adj.signif"}, label.size = 5,
                      step.group.by = 'host', y.position = c(25, 30)) +
  geom_text(data = p_worm, aes(label = paste0(method, ", p = ", p), y =32)) +
  labs(fill = "Population")
...
# Plot fecundity
plot_D <- 
  ggplot(subset(temp, pop != "Control"), aes(pop, fecundity/1000)) +
  geom_boxplot(aes(fill = pop)) +
  scale_fill_manual(values = my_color, limits = force) +
  ylab("Fecundity (x 1000)") +
  facet_wrap(~host, strip.position = 'bottom') + 
  geom_segment(data=fec[fec$host=="BALB/c",], aes(x=4.5, y=0, xend=4.5, yend=24), linetype="dashed",
               color = "black", size=0.5) +
  my_theme() +
  geom_text(data =fec_res, aes(label = paste0(method, ", p = ", p)), y = 24)  +
...
plot_E <- 
  ...
  facet_wrap(.~host, strip.position = 'bottom') + 
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=4.5, y=0, xend=4.5, yend=32000), linetype="dashed",
               color = "black", size=0.5) +
  my_theme() +
  #add_pvalue(p_organ, y.position = 27000, label.size =  4,  tip.length = 0, xmin = 'xmin', xmax = 'xmax') +
  add_pvalue(p_pop, y.position = c(28000, 29000, 30600), label.size =  5, tip.length = 0.01, color = "indianred") +
...
plot_ratio <- 
  ...
  facet_wrap(.~host, strip.position = 'bottom') + 
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=4.5, y=0, xend=4.5, yend=10), linetype="dashed",
               color = "black", size=0.5) +
...
(plot_A | plot_B) / plot_E / (plot_D  | plot_spacer()) + plot_annotation(tag_levels = c("A")) &
...
pdf("FigureS1.pdf", width = 5, height = 4)
