...
plot_lymph <- 
  ...
  geom_text(data = p_lymph_h1, aes(label = p.adj.signif), vjust = -0.5, y = 5) +
  geom_text_repel(
...
plot_HCT <- 
  ...
  geom_text(data = p_HCT_h1, aes(label = p.signif), vjust = -0.5, y = 1.5) +
  ylab("Mean HCT (%)") +
  labs(color = "Population") +
  scale_color_manual(values = my_color) +
  scale_x_discrete(labels=c("0","2","4","6", "8", "10")) +
...
plot_neut <- ggplot(CBC_summary, aes(time, `NEUT#(K/uL)_mean`, group = interaction(pop))) +
  ...
  geom_segment(data=CBC_all[CBC_all$host=="BALB/c",], aes(x=0, y=0, xend=6.5, yend=0), linetype="dashed",
               color = "black", size=0.5) +
  geom_text(data = f_NEUT_uni, aes(label = paste0('Friedman p = ', round(p, digits =10)), y = 0.75)) +
  geom_text(data = p_neut_h1, aes(label = p.signif), vjust = -0.5, y = 5) +
...
# Supplemental
pdf("FigureS3.pdf", width = 10, height = 8)

((plot_neut | plot_HCT) & theme(axis.title.x = element_blank())) / 
