...
plot_A <- 
  ...
  my_theme() +
  geom_text(data = fgain, aes(label = paste0('Friedman p = ', round(p, digits =10)), x = 4, y =25)) +
  geom_text_repel(
    aes(label = letter, color = pop), data = stat, position = position_dodge(width = 0.5), hjust = 0.5,
    show.legend = F) +
  geom_segment(data=weight_summary[weight_summary$host=="BALB/c",], aes(x=13.75, y=-1, xend = 13.75, yend=35), 
...
liver_res <- data.frame(host = c('BALB/c', 'C57BL/6'), test = c('ANOVA', 'Kruskal-Wallis'), p = c(0.003, 0.0205),
                        pop = 'EG')
...
plot_B <- 
  ...
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=5.5, y=0, xend=5.5, yend=15), linetype="solid",
               color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=1.5, y=0, xend=1.5, yend=14), linetype="dashed",
               color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="C57BL/6",], aes(x=1.5, y=0, xend=1.5, yend=14), linetype="dashed",
               color = "black", linewidth=0.5) +
  facet_wrap(~host, strip.position = 'bottom') + 
...
plot_C <- 
  ...
  facet_wrap(~host, strip.position = 'bottom') + 
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=5.5, y=0, xend=5.5, yend=6.5), linetype="solid",
               color = "black", size=0.5) +
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=1.5, y=0, xend=1.5, yend=6.5), linetype="dashed",
             color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="C57BL/6",], aes(x=1.5, y=0, xend=1.5, yend=6.5), linetype="dashed",
               color = "black", linewidth=0.5) +
  geom_text(data = spleen_res, aes(label = paste0('ANOVA', ', p = ',p)), y = 6.25) +
...
plot_S1 <- 
  ...
  facet_wrap(~host, strip.position = 'bottom') + 
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=5.5, y=300, xend=5.5, yend=550), linetype="solid",
               color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=1.5, y=300, xend=1.5, yend=550), linetype="dashed",
                 color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="C57BL/6",], aes(x=1.5, y=300, xend=1.5, yend=550), linetype="dashed",
                 color = "black", linewidth=0.5) +
  geom_text(data = intestine_res, aes(label = paste0('ANOVA = ', p), y= 550)) +
...
plot_D <- 
  ...
  facet_wrap(~host, strip.position = 'bottom') +
  ylab("Fibrotic area (%)") +
  geom_segment(data=stain_1[stain_1$host=="BALB/c",], aes(x=5.5, y=0, xend=5.5, yend=60), linetype="solid",
               color = "black", size=0.5) +
  geom_segment(data=all_df[all_df$host=="BALB/c",], aes(x=1.5, y=0, xend=1.5, yend=55), linetype="dashed",
               color = "black", linewidth=0.5) +
  geom_segment(data=all_df[all_df$host=="C57BL/6",], aes(x=1.5, y=0, xend=1.5, yend=55), linetype="dashed",
               color = "black", linewidth=0.5) +
  geom_text(data = kw_fibrosis, aes(label = paste0(method, ', p = ',p), y = 60)) +
...
plot_E <- 
  ...
  geom_segment(data=df_granulomas[df_granulomas$host=="BALB/c",], aes(x=4.5, y=0, xend=4.5, yend=2.5E05), linetype="dashed",
               color = "black", size=0.5) +
  geom_text(data = p_gran_h, aes(label = p.adj.signif), vjust = -0.5, y = 4) +
  geom_text(data = kw_granuloma, aes(label = paste0(method, ", p = ", p), y = 2.55E05) )+
  scale_fill_manual(values = my_color) +
  my_theme() +
  #theme(text = element_text(size =18)) +
  geom_text(data = gran_p, aes(label = letter), vjust = -0.5, y = 2.2E+05)  # +
...
#pdf("Figure3_ctrl_excluded.pdf", width = 10, height = 12)

#(plot_A + theme(legend.position = 'bottom')) / (plot_B | img_A) / (plot_C | img_B) / (plot_D | plot_E) +
...
pdf("FigureS2.pdf", width = 5, height = 4)
